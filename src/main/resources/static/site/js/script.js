...
(function () {
	...
	let initialDate = new Date(),
		...
		plugins = {
			...
			selectFilter: $("select"),
			onReadyActive:           $('.on-ready-active'),
			parallaxJs:              document.querySelectorAll('.parallax-scene-js'),
			letterToSpan:            document.querySelectorAll( '.letter-to-span' ),
		};
	...
	$(function () {
		...
		// Active class on window ready
		if ( plugins.onReadyActive.length ) {
			for ( let i = 0; i < plugins.onReadyActive.length; i++ ) {
				setTimeout( function () {
					plugins.onReadyActive[i].classList.add( 'active' );
				}, 1000 )
			}
		}

		// Letter to span
		if ( plugins.letterToSpan.length ) {
			for ( let i = 0; i < plugins.letterToSpan.length; i++ ) {
				plugins.letterToSpan[i].innerHTML = plugins.letterToSpan[i].innerHTML.split( '' ).map(v=>`<span>${v}</span>`).join("");
			}
		}
	});
}());
